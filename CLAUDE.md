...
**Critical difference from the code-graph rule above: there is no automatic hook for this by default.** It has to be called on purpose, every time. An agent that reads this rule once and then forgets it exists over a long session will quietly regress to raw grep, that has actually happened. If your harness supports a rule file (`.mdc` in Cursor, `alwaysApply: true`), that helps as a reminder but does not force the call the way a real hook does, don't confuse "there's a rule about it" with "it's enforced."
